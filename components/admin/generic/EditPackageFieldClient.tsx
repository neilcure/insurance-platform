    ...
    async function loadExisting() {
      ...
      try {
        ...
        if (current) {
          ...
          // Inherit groupShowWhen from siblings if not set on this field
          const groupName = String(mergedMeta.group ?? "").trim();
          if (groupName && !mergedMeta.groupShowWhen) {
            const sibling = rows.find((r) => r.id !== id && String((r.meta as any)?.group ?? "").trim() === groupName && (r.meta as any)?.groupShowWhen);
            if (sibling) mergedMeta.groupShowWhen = (sibling.meta as any).groupShowWhen;
          }
          setForm({
            label: current.label,
            value: current.value,
        ...
        <div className="grid gap-3">
                ...
                {(Array.isArray(form.meta?.options) ? (form.meta?.options ?? []) : []).map((opt: any, idx: number) => (
                  <div key={idx} className="grid grid-cols-12 items-center gap-2">
                    ...
                    <div className="col-span-12 mt-2 rounded-md border border-neutral-200 p-3 dark:border-neutral-800">
                      ...
                      <div className="grid gap-3">
                        {((Array.isArray(((form.meta?.options ?? []) as any[])[idx]?.children)
                          ? (((form.meta?.options ?? []) as any[])[idx]?.children ?? [])
                          : []) as any[]).map((child, cIdx) => (
                          <div key={`opt-child-${idx}-${cIdx}`} className="rounded-md border border-neutral-200 p-3 dark:border-neutral-800">
                            <div className="mb-2 flex items-center justify-between">
                                ...
                                <select
                                  ...
                                  onChange={(e) => {
                                    ...
                                    children[cIdx] = {
                                      ...(children[cIdx] ?? {}),
                                      inputType: nextType,
                                      options: nextType === "select" || nextType === "multi_select" ? (children[cIdx]?.options ?? []) : undefined,
                                    };
                                    next[idx] = { ...(next[idx] ?? {}), children };
                                    updateMeta("options", next as any);
                                  }}
                            ...
                            {/* Child boolean config */}
                            {child?.inputType === "boolean" ? (
                              <div className="col-span-12 mt-2 space-y-3">
                                <div className="grid gap-2 sm:grid-cols-2">
                                  <div className="grid gap-1">
                                    <Label>Yes Label</Label>
                                    <Input placeholder="Yes" value={String(child?.booleanLabels?.true ?? "")} onChange={(e) => {
                                      const next = [...((form.meta?.options ?? []) as any[])];
                                      const children = Array.isArray(next[idx]?.children) ? [...(next[idx]?.children ?? [])] : [];
                                  ...
                                  <div className="grid gap-1">
                                    ...
                                    <Input placeholder="No" value={String(child?.booleanLabels?.false ?? "")} onChange={(e) => {
                                      const next = [...((form.meta?.options ?? []) as any[])];
                                      const children = Array.isArray(next[idx]?.children) ? [...(next[idx]?.children ?? [])] : [];
                                ...
                                {(["true", "false"] as const).map((branch) => {
                                  ...
                                  const arr = Array.isArray(branchChildren) ? branchChildren : [];
                                  return (
                                    <div key={branch} className="space-y-2 rounded-md border border-neutral-200 p-2 dark:border-neutral-800">
                                      <div className="flex items-center justify-between">
                                        ...
                                        <Button type="button" size="sm" variant="secondary" onClick={() => {
                                          const next = [...((form.meta?.options ?? []) as any[])];
                                          const children = Array.isArray(next[idx]?.children) ? [...(next[idx]?.children ?? [])] : [];
                                          const bc = { ...(children[cIdx]?.booleanChildren ?? {}) };
                                          const bArr = Array.isArray(bc[branch]) ? [...bc[branch]] : [];
                                          bArr.push({ label: "", inputType: "string" });
                                          bc[branch] = bArr;
                                          children[cIdx] = { ...(children[cIdx] ?? {}), booleanChildren: bc };
                                          next[idx] = { ...(next[idx] ?? {}), children };
                                          updateMeta("options", next as any);
                                        }}>Add child</Button>
                                      </div>
                                      {arr.map((bc: any, bIdx: number) => (
                                        <div key={`opt-${idx}-c${cIdx}-b${branch}-${bIdx}`} className="rounded border border-neutral-100 p-2 dark:border-neutral-800">
                                          <div className="mb-1 flex items-center justify-between">
                                            <span className="text-xs font-medium">Child #{bIdx + 1}</span>
                                            <Button type="button" size="sm" variant="outline" onClick={() => {
                                              const next = [...((form.meta?.options ?? []) as any[])];
                                              const children = Array.isArray(next[idx]?.children) ? [...(next[idx]?.children ?? [])] : [];
                                              const bCh = { ...(children[cIdx]?.booleanChildren ?? {}) };
                                              const bArr = Array.isArray(bCh[branch]) ? [...bCh[branch]] : [];
                                              bArr.splice(bIdx, 1);
                                              bCh[branch] = bArr;
                                              children[cIdx] = { ...(children[cIdx] ?? {}), booleanChildren: bCh };
                                              next[idx] = { ...(next[idx] ?? {}), children };
                                              updateMeta("options", next as any);
                                            }}>Remove</Button>
                                          </div>
                                          <div className="grid grid-cols-12 gap-2">
                                            <div className="col-span-6">
                                              <Input placeholder="Label" value={String(bc?.label ?? "")} onChange={(e) => {
                                                const next = [...((form.meta?.options ?? []) as any[])];
                                                const children = Array.isArray(next[idx]?.children) ? [...(next[idx]?.children ?? [])] : [];
                                                const bCh = { ...(children[cIdx]?.booleanChildren ?? {}) };
                                                const bArr = Array.isArray(bCh[branch]) ? [...bCh[branch]] : [];
                                            ...
                                            <div className="col-span-6">
                                              <select className="h-10 w-full rounded-md border border-neutral-300 bg-white px-2 text-sm dark:border-neutral-700 dark:bg-neutral-900" value={bc?.inputType ?? "string"} onChange={(e) => {
                                                const next = [...((form.meta?.options ?? []) as any[])];
                                                const children = Array.isArray(next[idx]?.children) ? [...(next[idx]?.children ?? [])] : [];
                                                const bCh = { ...(children[cIdx]?.booleanChildren ?? {}) };
                                                const bArr = Array.isArray(bCh[branch]) ? [...bCh[branch]] : [];
                                            ...
                                            {bc?.inputType === "currency" ? (
                                              <>
                                                <div className="col-span-6">
                                                  <Input placeholder="e.g. HKD" value={String(bc?.currencyCode ?? "")} onChange={(e) => {
                                                    const next = [...((form.meta?.options ?? []) as any[])];
                                                    const children = Array.isArray(next[idx]?.children) ? [...(next[idx]?.children ?? [])] : [];
                                                    const bCh = { ...(children[cIdx]?.booleanChildren ?? {}) };
                                                    const bArr = Array.isArray(bCh[branch]) ? [...bCh[branch]] : [];
                                                ...
                                                <div className="col-span-6">
                                                  <Input type="number" placeholder="2" value={String(bc?.decimals ?? 2)} onChange={(e) => {
                                                    const next = [...((form.meta?.options ?? []) as any[])];
                                                    const children = Array.isArray(next[idx]?.children) ? [...(next[idx]?.children ?? [])] : [];
                                                    const bCh = { ...(children[cIdx]?.booleanChildren ?? {}) };
                                                    const bArr = Array.isArray(bCh[branch]) ? [...bCh[branch]] : [];
                                      ...
                                      {arr.length === 0 ? <p className="text-xs text-neutral-400">No children configured.</p> : null}
                                    </div>
                                  );
                                })}
                              </div>
                            ) : null}
                            {/* Child select/multi_select options */}
                            {child?.inputType && ["select", "multi_select"].includes(child.inputType) ? (
                              <div className="col-span-12 mt-2 space-y-2">
                                  ...
                                  <div className="flex gap-2">
                                    <Button
                                      ...
                                      onClick={() => {
                                        const next = [...((form.meta?.options ?? []) as any[])];
                                        const children = Array.isArray(next[idx]?.children) ? [...(next[idx]?.children ?? [])] : [];
                                        const childOptions = Array.isArray(children[cIdx]?.options) ? [...(children[cIdx]?.options ?? [])] : [];
                                ...
                                <div className="grid gap-2">
                                  {(((child?.options ?? []) as { label?: string; value?: string }[]) ?? []).map((copt, oi) => (
                                      ...
                                      <div className="col-span-5">
                                        <Input
                                          ...
                                          onChange={(e) => {
                                            const next = [...((form.meta?.options ?? []) as any[])];
                                            const children = Array.isArray(next[idx]?.children) ? [...(next[idx]?.children ?? [])] : [];
                                            const opts = Array.isArray(children[cIdx]?.options) ? [...(children[cIdx]?.options ?? [])] : [];
                            ...
                            {/* Cross-package condition for this child */}
                            <div className="mt-2">
                              <ShowWhenConfig
                                compact
                                value={Array.isArray(child?.showWhen) ? child.showWhen : []}
                                onChange={(next) => {
                                  const opts = [...((form.meta?.options ?? []) as any[])];
                                  const children = Array.isArray(opts[idx]?.children) ? [...(opts[idx]?.children ?? [])] : [];
                                  children[cIdx] = { ...(children[cIdx] ?? {}), showWhen: next };
                                  opts[idx] = { ...(opts[idx] ?? {}), children };
                  ...
                  <div className="rounded-md border border-neutral-200 p-3 dark:border-neutral-800">
                    ...
                    <div className="grid gap-3">
                      {(((form.meta?.booleanChildren as any)?.true ?? []) as any[]).map((child, cIdx) => (
                        <div key={`edit-bool-yes-${cIdx}`} className="rounded-md border border-neutral-200 p-3 dark:border-neutral-800">
                          <div className="mb-2 flex items-center justify-between">
                            <Label>Child #{cIdx + 1}</Label>
                            <Button
                              type="button"
                          ...
                          {/* Formula configuration for YES child */}
                          ...
                          {/* Currency configuration for YES child */}
                          {child?.inputType === "currency" ? (
                            <div className="col-span-12 mt-2 grid gap-2 sm:grid-cols-2">
                              <div className="grid gap-1">
                                <Label>Currency Code</Label>
                          ...
                          {/* Boolean configuration for YES child */}
                          {child?.inputType === "boolean" ? (
                            <div className="col-span-12 mt-2 space-y-3">
                              <div className="grid gap-2 sm:grid-cols-2">
                                <div className="grid gap-1">
                              ...
                              {/* Branch children for YES child boolean */}
                              {(["true", "false"] as const).map((branch) => {
                                const branchLabel = branch === "true" ? "When YES" : "When NO";
                                const branchChildren = (child?.booleanChildren as any)?.[branch] ?? [];
                                const arr = Array.isArray(branchChildren) ? branchChildren : [];
                                return (
                                  <div key={branch} className="space-y-2 rounded-md border border-neutral-200 p-2 dark:border-neutral-800">
                                    ...
                                    {arr.map((bChild: any, bIdx: number) => (
                                      <div key={`yes-c${cIdx}-b${branch}-${bIdx}`} className="rounded border border-neutral-100 p-2 dark:border-neutral-800">
                                        ...
                                        <div className="grid grid-cols-12 gap-2">
                                          ...
                                          {bChild?.inputType === "currency" ? (
                                            <>
                                              <div className="col-span-6">
                                                <Input placeholder="e.g. HKD" value={String(bChild?.currencyCode ?? "")} onChange={(e) => {
                                                  const bc = { ...((form.meta?.booleanChildren ?? {}) as any) };
                                                  const parentArr = Array.isArray(bc.true) ? [...(bc.true as any[])] : [];
                                                  const boolCh = { ...(parentArr[cIdx]?.booleanChildren ?? {}) };
                                                  const bArr = Array.isArray(boolCh[branch]) ? [...boolCh[branch]] : [];
                                              ...
                                              <div className="col-span-6">
                                                <Input type="number" placeholder="2" value={String(bChild?.decimals ?? 2)} onChange={(e) => {
                                                  const bc = { ...((form.meta?.booleanChildren ?? {}) as any) };
                                                  const parentArr = Array.isArray(bc.true) ? [...(bc.true as any[])] : [];
                                                  const boolCh = { ...(parentArr[cIdx]?.booleanChildren ?? {}) };
                                                  const bArr = Array.isArray(boolCh[branch]) ? [...boolCh[branch]] : [];
                          ...
                          {/* Repeatable configuration for YES child */}
                          ...
                          {/* Select/Multi-select options for YES child */}
                          {child?.inputType && ["select", "multi_select"].includes(child.inputType) ? (
                            <div className="col-span-12 mt-2 space-y-2">
                              <div className="flex items-center justify-between">
                                <Label>Options</Label>
                  ...
                  <div className="rounded-md border border-neutral-200 p-3 dark:border-neutral-800">
                    ...
                    <div className="grid gap-3">
                      {(((form.meta?.booleanChildren as any)?.false ?? []) as any[]).map((child, cIdx) => (
                        <div key={`edit-bool-no-${cIdx}`} className="rounded-md border border-neutral-200 p-3 dark:border-neutral-800">
                          <div className="mb-2 flex items-center justify-between">
                            <Label>Child #{cIdx + 1}</Label>
                            <Button
                              type="button"
                          ...
                          {/* Formula configuration for NO child */}
                          {child?.inputType === "formula" ? (
                            <div className="col-span-12 mt-2">
                              <Label>Formula Expression</Label>
                              <Input
                          ...
                          {/* Currency configuration for NO child */}
                          {child?.inputType === "currency" ? (
                            <div className="col-span-12 mt-2 grid gap-2 sm:grid-cols-2">
                              <div className="grid gap-1">
                                <Label>Currency Code</Label>
                          ...
                          {/* Boolean configuration for NO child */}
                          {child?.inputType === "boolean" ? (
                            <div className="col-span-12 mt-2 space-y-3">
                              <div className="grid gap-2 sm:grid-cols-2">
                                <div className="grid gap-1">
                              ...
                              {/* Branch children for NO child boolean */}
                              {(["true", "false"] as const).map((branch) => {
                                const branchLabel = branch === "true" ? "When YES" : "When NO";
                                const branchChildren = (child?.booleanChildren as any)?.[branch] ?? [];
                                const arr = Array.isArray(branchChildren) ? branchChildren : [];
                          ...
                          {/* Repeatable configuration for NO child */}
                          {String(child?.inputType ?? "") === "repeatable" ? (
                            <div className="col-span-12 mt-2 space-y-3 rounded-md border border-neutral-200 p-3 dark:border-neutral-800">
                              <div className="grid grid-cols-2 gap-2">
                                <div className="grid gap-1">
                          ...
                          {/* Select/Multi-select options for NO child */}
                          {child?.inputType && ["select", "multi_select"].includes(child.inputType) ? (
                            <div className="col-span-12 mt-2 space-y-2">
                              <div className="flex items-center justify-between">
                                <Label>Options</Label>
          ...
          <div className="grid gap-1">
            ...
            <div className="flex items-center gap-2">
              ...
              />
              <p className="text-xs text-neutral-500">Lower numbers appear first (within the same group).</p>
            </div>
          </div>
          <div className="grid gap-1">
            ...
            <select
              ...
              onChange={(e) => {
                ...
                if (v === "__custom") {
                  ...
                  if (v) {
                    const sibling = existing.find((r) => r.id !== id && String((r.meta as any)?.group ?? "").trim() === v && (r.meta as any)?.groupShowWhen);
                    if (sibling) {
