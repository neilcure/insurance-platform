/**
 * Resolve insurer master-policy IDs linked to a customer policy — same logic as
 * `GET /api/policies/[id]/linked-insurers`. Shared so dashboards can reuse it
 * without cookie-forwarding internal fetch().
 */
...
function extractRecordName(carExtra: Record<string, unknown>): string | null {
...
export async function resolveLinkedInsurerPolicyIds(policyId: number): Promise<number[]> {
