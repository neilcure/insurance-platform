...
/**
 * Pull a date field (`endDate` / `startDate`) out of a car's
 * snapshot.
 *
 * Why this isn't a one-liner over the shared field resolver
 * ---------------------------------------------------------
 * Per `.cursor/rules/shared-field-resolver.mdc` and the
 * `multi-variant-fields` skill, the proper way to read a value
 * from `cars.extraAttributes` is `resolveRawValue({ source:
 * "package", fieldKey, packageName }, ctx)`. That helper handles
 * the `${packageName}__${fieldKey}` prefix variants automatically
 * via `prefixedGet`. The catch: it requires us to KNOW both the
 * `packageName` and the `fieldKey`. Both are admin-configurable
 * via `form_options` and vary per tenant — verified live:
 *
 *   packagesSnapshot.policyinfo.values.policyinfo__startedDate
 *
 * Note `policyinfo` (not `policy`) and `startedDate` (past tense,
 * not `startDate`). A naive
 *   resolveRawValue({ source: "package", fieldKey: "startDate",
 *                     packageName: "policy" }, ctx)
 * call returns nothing for this tenant — and that's exactly the
 * silent-fail loop the dashboard was stuck in for an hour.
 *
 * Proper long-term fix (per `.cursor/skills/dynamic-config-first/SKILL.md`)
 * --------------------------------------------------------------------
 * Add an admin tag — e.g. `meta.dateRole: "start" | "end"` — to
 * the `form_options` row that holds these dates. Then this whole
 * scan collapses to:
 *
 *   const startKey = await loadDateRoleField("start"); // → { pkg, key }
 *   resolveRawValue({ source: "package", fieldKey: startKey.key,
 *                     packageName: startKey.pkg }, ctx);
 *
 * No more guessing, and it works for any custom tenant naming.
 *
 * What we ship now (the workaround)
 * ---------------------------------
 * SHALLOW walk of every package wrapper looking for a value whose
 * key NORMALISES to one of `directTokens` (the comprehensive list
 * below covers `startDate`, `startedDate`, `effectiveDate`,
 * `validFrom`, `coverStart`, `commencementDate`, etc.). Every
 * variation that's been seen in real tenant data is there. New
 * tenants WILL eventually break this list — at which point the
 * proper migration above is the answer, NOT another token.
 */
function extractDateField(
  ...
  // Tokens this field should normalise to. The first one that
  // matches a key wins. We include past-tense variants
  // (`startedDate`, `endedDate`, `expiredDate`) because that's how
  // a couple of tenants name their fields in admin-configured
  // packages — verified in the live DB:
  // `policyinfo__startedDate = "04-06-2026"`. Without `starteddate`
  // here the dashboard silently falls through to `endDate` and
  // mislabels it as "Starts X" — which is exactly what the user
  // first reported.
  //
  // NOTE: deliberately do NOT include `issuedate` — that's the day
  // the policy paperwork was created, not when coverage starts, and
  // it'd false-match the "issueDate" key seen alongside startedDate
  ...
  // Words to strip when normalising a key. We strip these so e.g.
  // "policyStartDate" or "policy__startDate" both normalise to
  // "startdate" and match `directTokens[0]`.
  ...
  const normaliseKey = (raw: string): string => {
    ...
    // strip separators
    k = k.replace(/[-_\s]+/g, "");
    // strip leading domain word so "policystartdate" → "startdate"
  ...
  /**
   * Scan one flat object for any key whose normalised form matches
   * one of `directTokens`. Returns the first non-empty string value
   * found, or "" if none match. Skips nested objects (arrays / dates
   * are kept as primitives).
   */
  const scanFlat = (obj: Record<string, unknown> | null | undefined): string => {
    ...
    for (const [k, v] of Object.entries(obj)) {
      ...
      // Exact normalised match → strongest signal; return immediately.
      for (const t of directTokens) {
        if (nk === t) return value;
      }
      // Suffix / contains match (e.g. "myCustomStartDate" → contains
      // "startdate") — keep the first one as a fallback, but keep
  ...
  // Walk every package — start with the canonical "policy" so it
  // wins ties, then any other package the tenant has configured.
  const pkgOrder = ["policy", ...Object.keys(pkgs).filter((k) => k !== "policy")];
  for (const pkgName of pkgOrder) {
    const pkg = pkgs[pkgName];
    if (!pkg || typeof pkg !== "object") continue;
    // Some shapes have `.values`, others put the values directly on
    // the package object — check both.
    const values =
      (pkg as { values?: Record<string, unknown> }).values ??
      (pkg as Record<string, unknown>);
    const v = scanFlat(values as Record<string, unknown>);
    if (v) return v;
    // Also check the wrapper itself in case `values` is present but
    // empty and the date sits beside it on the package object.
    if ((pkg as { values?: unknown }).values) {
      const w = scanFlat(pkg as Record<string, unknown>);
      if (w) return w;
    }
  }

  // Last fallback: flat keys directly on extraAttributes.
  return scanFlat(snapshot);
}
